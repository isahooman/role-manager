  ...
  execute(message, args) {
    ...
    try {
      if (arg === 'clear' || arg === 'refresh' || arg === 'reload') {
        ...
        cache.cacheRoles(message.client);
        ...
        const embed = new EmbedBuilder()
          ...
          .addFields(
            ...
            { name: 'Channels', value: `Total: ${cache.channels.size}\nThis guild: ${message.guild ? message.guild.channels.cache.size : 0}` },
            { name: 'Users', value: `Total: ${cache.members.size}\nThis guild: ${message.guild ? message.guild.memberCount : 0}` },
            { name: 'Threads', value: `Total: ${cache.threads.size}\nThis guild: ${message.guild ? message.guild.channels.cache.filter(channel => channel.isThread()).size : 0}` },
            { name: 'Roles', value: `Total: ${cache.roles.size}\nThis guild: ${message.guild ? message.guild.roles.cache.size : 0}` },
          )
        ...
        logger.info(`- Total Roles: ${cache.roles.size}`);
        ...
        if (message.guild) {
          ...
          logger.info(`- Roles: ${message.guild.roles.cache.size}`);
        }
      } else {
...
function getCacheSize(cacheInstance) {
  ...
  totalSize += inspect(cacheInstance.roles).length;
